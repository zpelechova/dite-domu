...
try:
    ...
    for i in range(1, 20):
        ...
        acount_id = random.randint(1,20)
        
        # definice query
        query = "INSERT INTO public.family(file_number, approval_type_id, regional_office_id,expectation_status_id, region_id, district_id, carer_info_id, prepcourse) VALUES ('" + file_number +"',"+ str(approval_type_id) + "," + str(regional_office_id) + "," + str(expectation_status_id) + "," + str(region_id) + "," + str(district_id) +"," + str(carer_info_id) + ",'" + prepcourse +"');"
        # spusteni query
